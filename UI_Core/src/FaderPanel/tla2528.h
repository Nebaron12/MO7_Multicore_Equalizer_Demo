...
// TLA2528 device struct
typedef struct {
    XIicPs *Iic;         // Pointer to initialized XIicPs instance
    uint16_t Address;    // 7-bit I2C address (left-aligned if needed)
} TLA2528;
...
//int TLA2528_ReadAllChannels(TLA2528 *dev, uint16_t *data, uint8_t baseAddress); // for reading 16-bit values (not working yet)
